...
for(int i=2;i<len;i++)
{
    ...
    return d[len-1];
        ...
        int[] d = new int[n];
        d[0] = c[0];
        d[1] = Math.max(c[0],c[1]);

        for (int i = 2; i < n; i++) {
            d[i] = Math.max(d[i - 1], d[i - 2] + c[i]);
        }
        ...
        int[] dp = new int[len];
        dp[0] = d[0][0];
        dp[1] = Math.max(d[0][0],d[0][1]);

        for (int i = 2; i < len; i++) {
            dp[i] = Math.max(dp[i - 1], dp[i - 2] +d[0][i]);
        }
  for (int t = 1; t <len; t++) {
            int[] currentType = d[t];
            int[] temp = new int[len];
            temp[0] = currentType[0];
            temp[1] = Math.max(currentType[0], currentType[1]);
        
         for (int i = 2; i < len; i++) {
                temp[i] = Math.max(temp[i - 1], temp[i - 2] + currentType[i]);
            }
              for (int i = 0; i < len; i++) {
                dp[i] = Math.max(dp[i], temp[i]);
            }
        }
        return dp[len - 1];
    }
...
//user inputed for RowHouses 
        ...
        int maxAmountRoundHouses = javaRobber.RowHouses(moneyInRoundHouses);
        System.out.println("Max amount robbed from row houses: " + maxAmountRoundHouses);

       


        //user inputed  for Square houses
